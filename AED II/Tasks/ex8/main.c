...
ArvBin insercao(ArvBin, int, int *);
...
int main() {
  ...
  scanf("%d", &aux);
  arvore = insercao(arvore, aux, &rAVL);
  ...
  while (aux >= 0) {
    ...
    if (aux >= 0) {
      ...
      temp = instancia_Arvore(aux);
      arv = insercao_AVP(arv, temp);
      verifica_erros(arv, temp, &rAVP, &rAVP, &mCorAVP);
      arv->cor = 0;
    } else {
      break;
    }
...
ArvBin insercao(ArvBin arvore, int num, int *rAVL) {
  if (arvore == NULL) {
    return instanciaArvore(num);
  }

  if (num < arvore->item.chave) {
    arvore->esq = insercao(arvore->esq, num, rAVL);
  } else if (num > arvore->item.chave) {
    arvore->dir = insercao(arvore->dir, num, rAVL);
  } else {
    return arvore;
  }
